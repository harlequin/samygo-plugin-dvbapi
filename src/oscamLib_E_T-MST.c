...
#define LIB_TV_MODELS "E T-MST"
#define LIB_HOOKS libOscamE_hooks
#define hCTX libOscamE_hook_ctx
		...
		for(int i = 0; i < MAX_DEMUX; i++)		
			for(int j = 0; j < MAX_DEMUX_FILTERS; j++)
				...
				{						
					...
					//log("Monitor stopped, dmxHandle=0x%08X, monHandle=0x%08X, flt=0x%02X, ret=0x%08X\n", g_demux[i].dmxHandle, g_demux[i].filter[j].hmon, j, hCTX.SdTSData_StopMonitor( g_demux[i].dmxHandle, g_demux[i].filter[j].hmon ));
	...
	{ 	
		...
		*request = ntohl(*request);
		...
		{				
			...
			g_demux[dmx].filter[flt].hmon = hCTX.SdTSData_StartMonitor( g_demux[dmx].dmxHandle, &g_demux[dmx].filter[flt].monParams, 0 );
