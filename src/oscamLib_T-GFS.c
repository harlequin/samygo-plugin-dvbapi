...
#define LIB_TV_MODELS "H T-GFS/T-GFP"
...
typedef struct 
{
    unsigned int eCipher;			// SD_CP_DESCRAMBLE  EQU 1
	unsigned int eType;				// SD_CP_DVBCSA     EQU 9
    unsigned int eMode;				// SD_CP_DEFAULT    EQU 0
    unsigned int eResBlock;			// SD_CP_CIP        EQU 0
	SdCPKeyInfo_t sOddKeyInfo;
	SdCPKeyInfo_t sEvenKeyInfo;    
	void* pSystemKey;
	void* pPid;
...
typedef union
{
	...
	struct
	{
		...
		int (*SdCP_Close)(unsigned int dmxHandle, unsigned int SdMainChip_k);
		int (*SdCP_Start)(unsigned int dmxHandle, SdCP_Param_t *pInfo, unsigned int SdMainChip_k, int bKeyTZProtected);
		int (*SdCP_Stop)(unsigned int dmxHandle, int pid, unsigned int SdMainChip_k);
		int (*SdCP_ChangeKeyInfo)(unsigned int dmxHandle, int bOddKey, SdCPKeyInfo_t *pKeyInfo, int pid, unsigned int SdMainChip_k, int bKeyTZProtected);
		...
		const int (*TCWindowImpl_GetRealSource)(void *window, unsigned int *ESource);
...
{	
	...
	(const void*)"_Z9SdCP_Initv",
	(const void*)"_Z9SdCP_Openj12SdMainChip_k",
	(const void*)"_Z10SdCP_Closej12SdMainChip_k",
	(const void*)"_Z10SdCP_StartjP12SdCP_Param_t12SdMainChip_ki",
	(const void*)"_Z9SdCP_Stopji12SdMainChip_k",
	(const void*)"_Z18SdCP_ChangeKeyInfojiP13SdCPKeyInfo_ti12SdMainChip_ki",
	...
	(const void*)"_ZN12TCWindowImpl13GetRealSourceERi",
...
{	
	...
	{		
		...
		memset(&dmx->filter[i].monParamsData.mask[1], 0, 2);
		memset(dmx->filter[i].monParamsData.mode, 0, MAX_FILTER_SIZE);
		
		dmx->filter[i].last_packet = malloc(sizeof(unsigned char) * PMT_SIZE);
		memset(dmx->filter[i].last_packet, 0, sizeof(unsigned char) * PMT_SIZE);
...
{		
	...
	if(sourceid == 0x11 || sourceid == 0x51)
	{
		...
		{
			log("SdCP_Stop=0x%08X\n", hCTX.SdCP_Stop(g_demux[dmxIndex].dmxHandle, 0, 0));
			log("SdCP_Close=0x%08X\n", hCTX.SdCP_Close(g_demux[dmxIndex].dmxHandle, 0));	
			log("SdCP_Open=0x%08X\n", hCTX.SdCP_Open(g_demux[dmxIndex].dmxHandle, 0));
		
			SdCP_Param_t param;
			memset(&param, 0, sizeof(SdCP_Param_t));
			
			unsigned char oddKey[8] = {0};
			unsigned char evenKey[8] = {0};
			
			param.eCipher = 1;
			param.eType = 9;
			param.sOddKeyInfo.pKey = oddKey;
			param.sOddKeyInfo.keyLen = 8;
			param.sEvenKeyInfo.pKey = evenKey;
			param.sEvenKeyInfo.keyLen = 8;
				
			log("SdCP_Start=0x%08X\n", hCTX.SdCP_Start(g_demux[dmxIndex].dmxHandle, &param, 0, 0));
		}
	}
}
	...
	if(len > 0 && len <= 1024)
	{
		...
		for(int i = 0; i < MAX_DEMUX; i++)		
			for(int j = 0; j < MAX_DEMUX_FILTERS; j++)
				if( match_filter( &g_demux[i].filter[j], pid, buf, len < MAX_FILTER_SIZE ? len : MAX_FILTER_SIZE ) 
					&& memcmp(g_demux[i].filter[j].last_packet, buf, len) )
				{						
					log("Got filter data, hmon=0x%08X, idx=0x%02X, flt=0x%02X, pid=0x%04X\n", hmon, i, j, pid);
					memset(g_demux[i].filter[j].last_packet, 0, sizeof(unsigned char) * PMT_SIZE);
					memcpy(g_demux[i].filter[j].last_packet, buf, len);					
					...
					//g_demux[i].filter[j].hmon += 0x8000000;
	...
	{ 	
		...
		*request = ntohl(*request);
		...
		else if (*request == CA_SET_DESCR)
		{
			...
			{
				SdCPKeyInfo_t keyInfo;
				memset(&keyInfo, 0, sizeof(SdCPKeyInfo_t));
				
				keyInfo.pKey = ca_descr.cw;
				keyInfo.keyLen = 8;
				
				log("SdCP_ChangeKeyInfo=0x%08X\n", hCTX.SdCP_ChangeKeyInfo(g_demux[adapter_index].dmxHandle, ca_descr.parity, &keyInfo, 0, 0, 0));
			}
		}		
		else if (*request == DMX_SET_FILTER)
		{				
			...
			if(g_demux[dmx].filter[flt].hmon != -1) 
				log("Monitor stopped, idx=0x%02X, flt=0x%02X, dmxHandle=0x%08X, monHandle=0x%08X, ret=0x%08X\n", dmx, flt, g_demux[dmx].dmxHandle, g_demux[dmx].filter[flt].hmon & 0x7FFFFFF, hCTX.SdTSData_StopMonitor( g_demux[dmx].dmxHandle, g_demux[dmx].filter[flt].hmon & 0x7FFFFFF, 0 ));
