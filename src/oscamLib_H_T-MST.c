...
{		
	...
	if(sourceid == 0x11 || sourceid == 0x51)
	{
		...
		{
			for(int i = 0; i < 16; i++)
				hCTX.MDrv_DSCMB2_FltFree(0, i);
		
			for(int i = 0; i < 16; i++)
			{		
				hCTX.MDrv_DSCMB2_FltAlloc(0);
				hCTX.MDrv_DSCMB2_FltConnectFltId(0, i, 16 + i);
				hCTX.MDrv_DSCMB2_FltTypeSet(0, i, 0);
			}
		}
	}
}
		...
		*request = ntohl(*request);
		...
		else if (*request == CA_SET_DESCR)
		{
			...
			if((adapter_index == 0 && g_demux[1].serviceId == -1) || adapter_index == 1)
				for(int i = 0; i < 16; i++)
					hCTX.MDrv_DSCMB2_FltKeySet(0, i, ca_descr.parity + 1, ca_descr.cw);
