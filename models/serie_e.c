...
typedef union {
	...
	struct	{
		...
		const int (*spITsd_DescramblerSetKey)(u32 hDesc, u32 DscmbId, u32 parity, u8 *data);
		void **g_pAppWindow;
...
int dvbapi_set_descriptor(ca_descr_t ca_descr) {
	g_fltDscmb = api_callbacks.spITsd_DescramblerSetKey(g_hDesc, g_DscmbId, ca_descr.parity, ca_descr.cw);
